...
function participationTag(status) {
...
export function TrainingActivitiesPage({ showToast }) {
  ...
  async function loadActivities() {
    const data = await api.get('/coach/training-activities');
    setActivities(data);
  }

  async function loadActivityTypes() {
    const data = await api.get('/coach/activity-types');
  ...
  useEffect(() => {
    loadActivities();
    loadActivityTypes();
    api.get('/coach/players').then(setPlayers);
  }, []);
  ...
  async function openView(a) {
    const participants = await api.get(`/coach/training-activities/${a.activity_id}/participation`);
  ...
  async function createActivityType() {
    ...
    try {
      const created = await api.post('/coach/activity-types', { name });
  ...
  async function saveEditType() {
    ...
    setSavingType(true);
    try {
      const updated = await api.patch(`/coach/activity-types/${editingTypeId}`, { name });
      await loadActivityTypes();
  ...
  async function deleteActivityType(t) {
    try {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <div style={{ display: 'flex', gap: 6 }}>
              <input
                ...
                autoFocus
              />
              <button type="button" className="btn btn-primary" onClick={createActivityType} disabled={creatingType || !newTypeName.trim()}>
          ...
          <tbody>
            {activities?.map((a) => (
              <tr key={a.activity_id}>
                ...
                <td>
                  <div style={{ display: 'flex', gap: 6 }}>
                    <button type="button" className="btn btn-secondary" onClick={() => openView(a)}>
                      Participants
                    </button>
        ...
        <DialogShell
          ...
          actions={
            <>
              ...
              <button type="button" className="btn btn-primary" onClick={submitLog} disabled={!name.trim()}>
                Log activity
              </button>
            </>
          }
                ...
                {players?.map((p) => (
                  <label key={p.player_id} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, padding: '4px 0' }}>
                    <input
                      ...
                      onChange={(e) =>
                        setParticipantIds((prev) => (e.target.checked ? [...prev, p.player_id] : prev.filter((id) => id !== p.player_id)))
                      }
                    />
