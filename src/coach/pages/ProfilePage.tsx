...
import type { CoachAnalytics, CoachProfile } from '../../api/domain';
...
export function ProfilePage({ showToast }: ProfilePageProps) {
  ...
  async function load() {
    const [p, a] = await Promise.all([
      api.get<CoachProfile>('/coach/profile'),
      api.get<CoachAnalytics>('/coach/analytics'),
    ]);
    setProfile(p);
  ...
  async function saveContact() {
    const updated = await api.patch<CoachProfile>('/coach/profile', { contact_number: contactNumber });
    setProfile(updated);
    setEditing(false);
  ...
  async function handlePhotoSelected(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const updated = await api.upload<CoachProfile>('/coach/profile/photo', formData);
      setProfile(updated);
  ...
  const statCards = analytics
    ? ([
        { label: 'Team size', value: analytics.total_players, icon: PlayersIcon, variant: 'info' as const },
        { label: 'Training activities', value: analytics.total_activities, icon: ActivitiesIcon, variant: 'success' as const },
        { label: 'Team attendance rate', value: `${analytics.attendance_rate}%`, icon: AttendanceIcon, variant: 'warning' as const },
        { label: 'Average rating given', value: analytics.average_rating != null ? analytics.average_rating.toFixed(1) : '—', icon: StarIcon, variant: 'accent' as const },
      ])
    : [];
    ...
    <>
      ...
      <div className="card elev-sm" style={{ padding: 24, maxWidth: 560, marginBottom: 20 }}>
        ...
        <p className="card-body" style={{ marginTop: 14 }}>
          Only your contact number and photo can be updated here. Contact an administrator to update your name,
          email, or team assignment.
        </p>
      </div>
    </>
  );
