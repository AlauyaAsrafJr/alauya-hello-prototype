...
export function PerformanceFeedbackPage({ showToast }) {
  ...
  async function loadFeedback() {
    const data = await api.get('/coach/performance-feedback');
    setFeedback(data);
  }

  async function loadCategories() {
    const data = await api.get('/coach/feedback-categories');
    setCategories(data);
  }
  ...
  async function createCategory() {
    ...
    try {
      const created = await api.post('/coach/feedback-categories', { name });
  ...
  async function saveEditCategory() {
    ...
    try {
      const updated = await api.patch(`/coach/feedback-categories/${editingCategoryId}`, { name });
      await loadCategories();
        ...
        <div className="card elev-sm" style={{ padding: 20, flex: '1 1 480px', maxWidth: 640 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {[1, 2, 3, 4, 5].map((n) => (
                  <button
                    ...
                    style={{
                      color: n <= rating ? 'var(--color-accent)' : 'var(--color-divider)',
                      border: '1px solid var(--color-divider)',
                    }}
                    onClick={() => setRating(n)}
                    aria-label={`${n} star`}
            ...
            <button
              ...
              className="btn btn-primary"
              onClick={submit}
              disabled={submitting || !playerId || !comments.trim()}
