...
export function ParticipationPage() {
  const [records, setRecords] = useState<ParticipationRecord[] | null>(null);

  useEffect(() => {
    api.get<ParticipationRecord[]>('/player/participation').then(setRecords);
  }, []);
        ...
        <tbody>
          {records.map((r) => (
            <tr key={r.participation_id}>
              ...
              <td><span className={statusTag(r.participation_status)}>{r.participation_status.replace('_', ' ')}</span></td>
            </tr>
          ))}
          {records.length === 0 && (
            <tr>
              <td colSpan={3} style={{ opacity: 0.6, textAlign: 'center', padding: 24 }}>No participation history yet.</td>
            </tr>
          )}
        </tbody>
      </table>
